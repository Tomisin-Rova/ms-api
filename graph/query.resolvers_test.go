...
func TestQueryResolver_Accounts(t *testing.T) {
	...
	assert.Equal(t, accounts.Nodes[0].AccountData.Balances.TotalBalance, &mockAccounts[0].AccountData.Balances.TotalBalance)
...
func TestQueryResolver_TransferFees(t *testing.T) {
	...
	assert.Equal(t, mockFee.Fees[0].Fee, fees.Fees[0].Fee)
}
