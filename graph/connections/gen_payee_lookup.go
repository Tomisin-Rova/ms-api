...
// TypesPayeeEdgeMaker define a function that take a *types.Payee and an offset and
// create an Edge.
type PayeeConnectionEdgeMaker func(value *types.Payee, offset int) Edge

// PayeeConnectionConMaker define a function that create a types.PayeeConnection
type PayeeConnectionConMaker func(
	edges []*types.PayeeEdge,
	nodes []*types.Payee,
	info *types.PageInfo,
	totalCount int) (*types.PayeeConnection, error)

// PayeeConnectionCon will paginate a source according to the input of a relay connection
func PayeeConnectionCon(source []*types.Payee, edgeMaker PayeeConnectionEdgeMaker, conMaker PayeeConnectionConMaker, input models.ConnectionInput) (*types.PayeeConnection, error) {
	var nodes []*types.Payee
	var edges []*types.PayeeEdge
	var cursors []string
	...
	if input.Before != nil {
		for i, value := range source {
			...
			e := edge.(types.PayeeEdge)
			edges = append(edges, &e)
			cursors = append(cursors, edge.GetCursor())
			nodes = append(nodes, value)
		}
	} else {
		edges = make([]*types.PayeeEdge, len(source))
