...
// TypesPersonEdgeMaker define a function that take a *types.Person and an offset and
// create an Edge.
type PeopleLookupEdgeMaker func(value *types.Person, offset int) Edge

// PeopleLookupConMaker define a function that create a types.PersonConnection
type PeopleLookupConMaker func(
	edges []*types.PersonEdge,
	nodes []*types.Person,
	info *types.PageInfo,
	totalCount int) (*types.PersonConnection, error)

// PeopleLookupCon will paginate a source according to the input of a relay connection
func PeopleLookupCon(source []*types.Person, edgeMaker PeopleLookupEdgeMaker, conMaker PeopleLookupConMaker, input models.ConnectionInput) (*types.PersonConnection, error) {
	var nodes []*types.Person
	var edges []*types.PersonEdge
	var cursors []string
	...
	if input.Before != nil {
		for i, value := range source {
			...
			e := edge.(types.PersonEdge)
			edges = append(edges, &e)
			cursors = append(cursors, edge.GetCursor())
			nodes = append(nodes, value)
		}
	} else {
		edges = make([]*types.PersonEdge, len(source))
