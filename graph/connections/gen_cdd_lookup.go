...
// TypesCddEdgeMaker define a function that take a *types.Cdd and an offset and
// create an Edge.
type CddLookupEdgeMaker func(value *types.Cdd, offset int) Edge

// CddLookupConMaker define a function that create a types.CDDConnection
type CddLookupConMaker func(
	edges []*types.CDDEdge,
	nodes []*types.Cdd,
	info *types.PageInfo,
	totalCount int) (*types.CDDConnection, error)

// CddLookupCon will paginate a source according to the input of a relay connection
func CddLookupCon(source []*types.Cdd, edgeMaker CddLookupEdgeMaker, conMaker CddLookupConMaker, input models.ConnectionInput) (*types.CDDConnection, error) {
	var nodes []*types.Cdd
	var edges []*types.CDDEdge
	var cursors []string
	...
	if input.Before != nil {
		for i, value := range source {
			...
			e := edge.(types.CDDEdge)
			edges = append(edges, &e)
			cursors = append(cursors, edge.GetCursor())
			nodes = append(nodes, value)
		}
	} else {
		edges = make([]*types.CDDEdge, len(source))
