...
// TypesTagEdgeMaker define a function that take a *types.Tag and an offset and
// create an Edge.
type TagConnectionEdgeMaker func(value *types.Tag, offset int) Edge

// TagConnectionConMaker define a function that create a types.TagConnection
type TagConnectionConMaker func(
	edges []*types.TagEdge,
	nodes []*types.Tag,
	info *types.PageInfo,
	totalCount int) (*types.TagConnection, error)

// TagConnectionCon will paginate a source according to the input of a relay connection
func TagConnectionCon(source []*types.Tag, edgeMaker TagConnectionEdgeMaker, conMaker TagConnectionConMaker, input models.ConnectionInput) (*types.TagConnection, error) {
	var nodes []*types.Tag
	var edges []*types.TagEdge
	var cursors []string
	...
	if input.Before != nil {
		for i, value := range source {
			...
			e := edge.(types.TagEdge)
			edges = append(edges, &e)
			cursors = append(cursors, edge.GetCursor())
			nodes = append(nodes, value)
		}
	} else {
		edges = make([]*types.TagEdge, len(source))
