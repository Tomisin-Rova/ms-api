 ...
 `[1:], filepath.Base(os.Args[0]))
	...
	list := func() error {
		...
		targets := map[string]string{
			"genProto*": "Manage your deps, or running package managers.",
			"genSchema": "",
		}
	...
	for _, target := range args.Args {
		...
		switch strings.ToLower(target) {
			...
			case "genproto":
				if args.Verbose {
					logger.Println("Running target:", "GenProto")
				}
							wrapFn := func(ctx context.Context) error {
				return GenProto()
			}
			...
			case "genschema":
				if args.Verbose {
					logger.Println("Running target:", "GenSchema")
				}
							wrapFn := func(ctx context.Context) error {
				return GenSchema()
			}
